...
uniform float4 _CloudsCoverageSettings; //x = _GlobalCoverage, y = Bottom Coverage Mod, z = Top coverage mod
...
float CalculateCloudDensity(float3 pos, float3 PlanetCenter, float3 weather, float mip, float dist, bool details)
{
	...
	// Animate Wind
	coord.xyz += float3(_CloudsAnimation.x, -_Time.x * 0.5, _CloudsAnimation.y);
...
float GetLightEnergy(float3 p, float height_fraction, float dl, float ds_loded, float phase_probability, float cos_angle, float step_size, float brightness, float view)
{
	...
	float attenuation_probability = Remap(1 - cos_angle, 0.1, 1.0, prim_att, sec_att);
	float vertical_probability = pow(Remap(height_fraction, 0.07, 0.14, 0.3, 1.0), 0.8);
	float depth_probability = lerp(0.05 + pow(ds_loded, Remap(height_fraction, 0.3, 0.85, _CloudsLightingExtended.x * 0.5, _CloudsLightingExtended.x * 5)), 1.0, saturate(dl / step_size));
...
float GetDensityAlongRay(float3 pos, float3 PlanetCenter, float3 LightDirection, float3 weather, float dist, float h)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (opticalDepth < 0.3)
			opticalDepth += CalculateCloudDensity(pos, PlanetCenter, weather, mip_offset, dist, true) * densMult;
		else
			opticalDepth += CalculateCloudDensity(pos, PlanetCenter, weather, mip_offset, dist, false) * densMult;
