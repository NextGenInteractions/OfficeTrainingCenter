...
public class EnviroSkyRendering : MonoBehaviour
{
    ...
    [HideInInspector]public bool isAddionalCamera = false;
   ...
   // private float _threshold = 0f;
    ...
    public void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        else
        {
            ...
            if (useVolumeClouds)
            {
                if (!Application.isPlaying && EnviroSky.instance.showVolumeCloudsInEditor || Application.isPlaying)
                {
                    RenderVolumeClouds(tempTexture, tempTexture2);
                }
                else
                {
    ...
    private void RenderVolumeClouds(RenderTexture src, RenderTexture dst)
    {
        ...
        int downsampling = usedCloudsQuality.bilateralUpsampling ? reprojectionPixelSize * usedCloudsQuality.cloudsRenderResolution : 1;
    ...
    private void RenderVolumeFog(RenderTexture src, RenderTexture dst)
    {
        ...
        if (EnviroSky.instance.volumeLightSettings.dirVolumeLighting)
        {
            ...
            else
            {
                ...
                // volumeLightMat.SetMatrix("_MyLightMatrix0", _light.transform.localToWorldMatrix);
            }
    ...
    private void SetCloudProperties()
    {
        ...
        float toneMap = EnviroSky.instance.cloudsSettings.tonemapping ? 0f : 1f;
        ...
        if (myCam.transform.position.y < bottomH - 250)
            cloudsMat.SetVector("_CloudsParameter", new Vector4(bottomH, topH, topH - bottomH, EnviroSky.instance.cloudsSettings.cloudsWorldScale * 10));
        else
            cloudsMat.SetVector("_CloudsParameter", new Vector4(myCam.transform.position.y + 250, topH + ((myCam.transform.position.y + 250) - bottomH), (topH + ((myCam.transform.position.y + 250) - bottomH)) - (myCam.transform.position.y + 250), EnviroSky.instance.cloudsSettings.cloudsWorldScale * 10));
        ...
        cloudsMat.SetVector("_CloudsErosionIntensity", new Vector4(1f - EnviroSky.instance.cloudsConfig.baseErosionIntensity, EnviroSky.instance.cloudsConfig.detailErosionIntensity,0f,0f));
